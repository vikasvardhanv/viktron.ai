    ...
    <Layout>
      ...
      <section className="pt-32 pb-10 px-4">
        <div className="container-custom">
          <AnimatedSection>
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">
              <ShoppingCart className="h-4 w-4 text-blue-600" />
              Workflow Store
            </div>
            <h1 className="mt-6 text-5xl sm:text-7xl font-semibold tracking-tight text-slate-900">Ready-to-deploy workflows.</h1>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-3xl font-semibold text-slate-900">{selectedWorkflow.name}</h2>
                  <p className="mt-3 max-w-3xl text-slate-600 leading-relaxed">
                    {selectedWorkflow.description || 'No description provided.'}
                  </p>
                  <div className="mt-4 flex items-center gap-2">
                    <span className="text-3xl font-semibold text-slate-700">$39</span>
              ...
              {isAuthenticated && isPurchased && selectedWorkflow.hasInstructions ? (
                <div className="mt-7 rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-slate-900">Setup instructions</p>
                  ...
                  {showInstructions ? (
                    <div className="mt-4 text-sm leading-relaxed text-slate-700">
                      {loadingInstructions ? (
                        <p className="text-slate-600">Loading instructions…</p>
                      ) : instructionsError ? (
                        <p className="text-red-600">{instructionsError}</p>
                      ) : instructionsMd ? (
                        <SafeMarkdown assetBaseUrl={getStoreAssetsBaseUrl() || undefined}>{instructionsMd}</SafeMarkdown>
                      ) : (
                        <p className="text-slate-600">No instructions found.</p>
                      )}
                    </div>
                  ) : null}
                </div>
            ...
            <>
              <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-3xl font-semibold text-slate-900">{categoryDetail.title}</h2>
                  <p className="mt-1 text-sm text-slate-600">{categoryDetail.workflows.length} workflows listed</p>
                </div>
                <Link to="/store">
              ...
              <div className="space-y-3">
                {(categoryDetail.workflows || []).slice(0, visibleWorkflows).map((workflow) => (
                  <Link key={`${workflow.categorySlug}:${workflow.fileName}`} to={`/store/${categoryDetail.slug}/${encodeFile(workflow.fileName)}`}>
                    <article className="rounded-2xl border border-slate-200 bg-white p-4 hover:bg-slate-50 transition">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <h3 className="text-lg font-semibold text-slate-900">{workflow.name}</h3>
                          <p className="mt-1 max-w-4xl text-sm text-slate-600 line-clamp-2">
                        ...
                        <div className="text-right">
                          <p className="text-xl font-semibold text-slate-700">$39</p>
              ...
              <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5">
                <h3 className="text-xl font-semibold text-slate-900">Need a custom workflow?</h3>
                  ...
                  <StaggerItem key={category.slug}>
                    <Link to={`/store/${category.slug}`} className="block h-full">
                      <div className="card h-full p-5">
                        ...
                        <p className="mt-2 text-sm text-slate-600">{(category.count ?? 0).toLocaleString()} workflows</p>
